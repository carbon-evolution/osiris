...
// Build a threat-association graph for a clicked malware / threat-intel IP.
// Mirrors the map's mesh: the clicked IP and its family, sibling IPs of the
// same family, and every other threat source (Live Malware families +
// AbuseIPDB / Blocklist.de / SSL Blacklist / Phishing) all linked into a
// central MITRE ATT&CK hub with tracked APT groups.
function buildThreatGraph(clicked: any, data: any) {
  const ip: string = clicked?.ip || '';
  const family: string = (clicked?.threat_type || clicked?.malware || 'Unknown Threat').trim() || 'Unknown Threat';
  const nodes: any[] = [];
  const links: any[] = [];
  const seen = new Set<string>();
  const addNode = (n: any) => { if (!seen.has(n.id)) { seen.add(n.id); nodes.push(n); } };
  const addLink = (source: string, target: string, label: string) => links.push({ source, target, label });

  const root = `ip:${ip}`;
  addNode({ id: root, label: ip, type: 'ip', properties: { threat_type: family, status: clicked?.status || '' } });

  // Normalised pool of all threats across the loaded sources.
  const pool: any[] = [
    ...((data?.malware_threats || []).map((t: any) => ({ ip: t.ip, fam: t.malware || t.threat_type || 'Live Malware', country: t.country }))),
    ...((data?.threat_intel || []).map((t: any) => ({ ip: t.ip, fam: t.malware || t.threat_type || 'Threat Intel', country: t.country }))),
  ].filter((t) => t.ip);

  // The clicked node's family.
  const famId = `event:${family}`;
  addNode({ id: famId, label: family, type: 'event', properties: { role: 'Threat family / source' } });
  addLink(root, famId, 'member of');

  // Sibling IPs in the same family (the interconnected web).
  const siblings = pool.filter((t) => t.ip !== ip && t.fam === family).slice(0, 6);
  for (const s of siblings) {
    const sid = `ip:${s.ip}`;
    addNode({ id: sid, label: s.ip, type: 'ip', properties: { family: s.fam, country: s.country } });
    addLink(sid, famId, 'member of');
    if (s.country) {
      const cid = `country:${s.country}`;
      addNode({ id: cid, label: s.country, type: 'country', properties: {} });
      addLink(sid, cid, 'hosted in');
    }
  }

  // Central MITRE ATT&CK hub linking every source family together.
  const mitreId = 'apt:MITRE';
  addNode({ id: mitreId, label: 'MITRE ATT&CK', type: 'apt', properties: { role: 'Adversary TTP framework' } });
  addLink(famId, mitreId, 'mapped to');

  // Other distinct source families, ranked by prevalence, linked to the hub.
  const counts: Record<string, number> = {};
  for (const t of pool) counts[t.fam] = (counts[t.fam] || 0) + 1;
  const otherFams = Object.keys(counts).filter((f) => f !== family).sort((a, b) => counts[b] - counts[a]).slice(0, 8);
  for (const f of otherFams) {
    const fid = `event:${f}`;
    addNode({ id: fid, label: f, type: 'event', properties: { observed: counts[f] } });
    addLink(fid, mitreId, 'mapped to');
  }

  // Tracked APT groups from the cyber-intel feed.
  const groups = (data?.cyber_intel?.mitre_enrichment?.tactical_groups || []).slice(0, 4);
  for (const g of groups) {
    const gid = `apt:${g.id}`;
    addNode({ id: gid, label: g.name || g.id, type: 'apt', properties: { country: g.country, techniques: (g.techniques || []).join(', ') } });
    addLink(mitreId, gid, 'tracked group');
  }
  ...
  useEffect(() => {
    (window as any).openOsirisIntel = (entity: any) => {
      if (entity?.callsign || entity?.icao24) {
        ...
        const tg = buildThreatGraph(entity, dataRef.current);
        setEntityGraphTarget({
          type: 'ip', id: entity.ip, label: entity.ip,
          properties: { threat_type: entity.threat_type, status: entity.status },
          ...(tg.nodes.length > 1 ? { seedGraph: tg } : {}),
        });
        setShowEntityGraph(true);
    ...
    <main className="fixed inset-0 w-full h-full bg-[var(--bg-void)] overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 2.5 }} className={`absolute top-4 z-[200] pointer-events-none flex flex-col`} style={{ left: isMobile ? '24px' : '96px', right: '24px' }}>
        <div className="flex items-center gap-3 w-fit">
          <svg viewBox="0 0 650 500" className="w-8 h-8 md:w-10 md:h-10 shrink-0 transition-colors duration-500 text-[#1A73E8] drop-shadow-[0_0_8px_rgba(255,215,0,0.5)]" fill="currentColor">
          ...
          <div className="flex flex-col items-start gap-0.5">
            <h1 className="text-lg md:text-xl font-bold tracking-[0.4em] text-[#1A73E8] font-mono">OSIRIS</h1>
            <span className="text-[8px] md:text-[9px] font-mono tracking-[0.2em] opacity-80 uppercase text-[#1A73E8]">GLOBAL INTELLIGENCE COMMAND</span>
          </div>
        </div>
