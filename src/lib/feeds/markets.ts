/**
 * OSIRIS — Financial markets feed fetcher (pure; no Next types).
 * Defense stocks, oil, commodities, crypto, indices via Yahoo Finance + CoinGecko,
 * plus chokepoint→commodity SCM alerts derived from the maritime feed.
 */
...
export async function fetchMarkets(): Promise<MarketsPayload> {
  ...
  for (const [name, data] of Object.entries(cgCrypto)) { if (!crypto[name]) crypto[name] = data; }
  const indices: Record<string, any> = {};
  for (const { symbol, data } of indexResults) { if (data) indices[INDEX_NAMES[symbol] || symbol] = data; }

  // SCM: chokepoint → commodity correlation (best-effort; maritime served from same app).
  ...
  } catch { /* maritime unreachable — alerts optional */ }

  // Total upstream failure (no quotes at all) → throw so cacheFirst keeps the last snapshot.
  const anyData = Object.keys(stocks).length + Object.keys(oil).length + Object.keys(commodities).length
    + Object.keys(crypto).length + Object.keys(indices).length;
  if (anyData === 0) throw new Error('all market sources unavailable');

  return { stocks, oil, commodities, crypto, indices, scm_alerts, timestamp: new Date().toISOString() };
}
